...
class FileCacheMode(str, Enum):
    """Enumeration of the modes available for for the ppathlib file cache.

    Attributes:
        persistent (str): Cache is not removed by `ppathlib`.
    ...
    Modes can be set by passing them to the Client or by setting the `PPATHLIB_FILE_CACHE_MODE`
    ...
    def from_environment(cls) -> Optional["FileCacheMode"]:
        """Parses the environment variable `PPATHLIB_FILE_CACHE_MODE` into
        ...
        env_string = os.environ.get("PPATHLIB_FILE_CACHE_MODE", "").lower()

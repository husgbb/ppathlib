...
from ppathlib.exceptions import (
...
def test_environment_variable_instantiation(rig: CloudProviderTestRig, tmpdir):
    ...
    original_env_setting = os.environ.get("PPATHLIB_FILE_CACHE_MODE", "")

    try:
        for v in FileCacheMode:
            os.environ["PPATHLIB_FILE_CACHE_MODE"] = v.value
            local = tmpdir if v == FileCacheMode.persistent else None
            client = rig.client_class(local_cache_dir=local, **rig.required_client_kwargs)
            assert client.file_cache_mode == v

    finally:
        os.environ["PPATHLIB_FILE_CACHE_MODE"] = original_env_setting


def test_environment_variable_local_cache_dir(rig: CloudProviderTestRig, tmpdir):
    # environment instantiation
    original_env_setting = os.environ.get("PPATHLIB_LOCAL_CACHE_DIR", "")

    try:
        os.environ["PPATHLIB_LOCAL_CACHE_DIR"] = tmpdir.strpath
        client = rig.client_class(**rig.required_client_kwargs)
        ...
        os.environ["PPATHLIB_LOCAL_CACHE_DIR"] = ""
        client = rig.client_class(**rig.required_client_kwargs)
        assert client._cache_tmp_dir is not None

    finally:
        os.environ["PPATHLIB_LOCAL_CACHE_DIR"] = original_env_setting
...
def test_environment_variables_force_overwrite_from(rig: CloudProviderTestRig, tmpdir):
    # environment instantiation
    original_env_setting = os.environ.get("PPATHLIB_FORCE_OVERWRITE_FROM_CLOUD", "")

    try:
        # explicitly false overwrite
        os.environ["PPATHLIB_FORCE_OVERWRITE_FROM_CLOUD"] = "False"
        ...
        for val in ["1", "True", "TRUE"]:
            os.environ["PPATHLIB_FORCE_OVERWRITE_FROM_CLOUD"] = val
        ...
        os.environ["PPATHLIB_FORCE_OVERWRITE_FROM_CLOUD"] = original_env_setting


@pytest.mark.flaky(reruns=3, reruns_delay=1, condition=os.getenv("USE_LIVE_CLOUD") == "1")
def test_environment_variables_force_overwrite_to(rig: CloudProviderTestRig, tmpdir):
    # environment instantiation
    original_env_setting = os.environ.get("PPATHLIB_FORCE_OVERWRITE_TO_CLOUD", "")

    try:
        # explicitly false overwrite
        os.environ["PPATHLIB_FORCE_OVERWRITE_TO_CLOUD"] = "False"
        ...
        for val in ["1", "True", "TRUE"]:
            os.environ["PPATHLIB_FORCE_OVERWRITE_TO_CLOUD"] = val
        ...
        os.environ["PPATHLIB_FORCE_OVERWRITE_TO_CLOUD"] = original_env_setting
